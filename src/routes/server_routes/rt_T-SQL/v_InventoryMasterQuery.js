...
export async function post(req, res, next) {
  console.log(
    `hello from within the async function of v_InventoryMasterQuery.js`
  );
  ...
  async function showcatapultResults(result) {
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      if (
        actlMargRangeLessVal !== "enter value" &&
        actlMargRangeGreaterVal !== "enter value"
      ) {
        //if you've provided some
        ...
        if (
          catapultResObj["actlMarg"] < parseFloat(actlMargRangeLessVal) &&
        ...
        if (actlMargRangeLessVal !== "enter value") {
          //if you've provided some logic via the actualMargLessRange input (i.e. '<0')
          //use that logic to filter the result set for only items that meet the actual margin criteria
          ...
          if (actlMargRangeGreaterVal !== "enter value") {
            //if you've provided some logic via the actualMargGreaterRange input (i.e. '>0')
            //use that logic to filter the result set for only items that meet the actual margin criteria
            if (
              catapultResObj["actlMarg"] > parseFloat(actlMargRangeGreaterVal)
            ) {
            ...
            //otherwise, display entire result set, regardless of actual margin values
            queryResArr.push(catapultResObj);
            srcRsXLS_tsql.push(catapultResObj);
          }
        }
  ...
  async function paginCalcs() {
    //we are hard-coding page length to 100 results per page for now
