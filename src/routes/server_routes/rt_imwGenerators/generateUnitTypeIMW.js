...
export async function post(req, res, next) {
  ...
  function showSearchRes(rows) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      srsObj["pkg_qty"] = "";
      if (skuToggle === "edi") {
        //toggle for EDI versus Catapult SKU. This seems like the best place to catch any SKU mismatches,
      ...
      if (rows[i]["ord_supplierstocknumber"] !== "") {
        //don't include results with empty SKUs
  ...
  connection
    .query(query, function (err, rows, fields) {
      if (err) throw err;
      showSearchRes(rows);
    })
    .on("end", function () {
