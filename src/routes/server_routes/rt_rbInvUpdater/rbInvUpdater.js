                ...
                calcResRbInvUpdater().then(function () {
                  ...
                  function displayRbInvJoin(rows) {
                    ...
                    for (let i = 0; i < indRows.length; i++) {
                      ...
                      rbInvJoinObj_ind["new_inv_upcIND"] =
                        indRows[i]["new_inv_upc"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_ind["new_inv_nameIND"] =
                        indRows[i]["new_inv_name"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                    ...
                    for (let i = 0; i < smRows.length; i++) {
                      ...
                      rbInvJoinObj_sm["new_inv_upcSM"] =
                        smRows[i]["new_inv_upc"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_sm["new_inv_nameSM"] =
                        smRows[i]["new_inv_name"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_sm["new_inv_sm_stockSM"] =
                        smRows[i]["new_inv_sm_stock"];
                      rbInvJoinObj_sm["old_inv_sm_stockSM"] =
                        smRows[i]["old_inv_sm_stock"];
                    ...
                    for (let i = 0; i < mtRows.length; i++) {
                      ...
                      rbInvJoinObj_mt["new_inv_upcMT"] =
                        mtRows[i]["new_inv_upc"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_mt["new_inv_nameMT"] =
                        mtRows[i]["new_inv_name"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_mt["new_inv_mt_stockMT"] =
                        mtRows[i]["new_inv_mt_stock"];
                      rbInvJoinObj_mt["old_inv_mt_stockMT"] =
                        mtRows[i]["old_inv_mt_stock"];
                    ...
                    for (let i = 0; i < shRows.length; i++) {
                      ...
                      rbInvJoinObj_sh["new_inv_upcSH"] =
                        shRows[i]["new_inv_upc"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_sh["new_inv_nameSH"] =
                        shRows[i]["new_inv_name"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_sh["new_inv_sh_stockSH"] =
                        shRows[i]["new_inv_sh_stock"];
                      rbInvJoinObj_sh["old_inv_sh_stockSH"] =
                        shRows[i]["old_inv_sh_stock"];
                    ...
                    for (let i = 0; i < glRows.length; i++) {
                      ...
                      rbInvJoinObj_gl["new_inv_upcGL"] =
                        glRows[i]["new_inv_upc"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_gl["new_inv_nameGL"] =
                        glRows[i]["new_inv_name"]; //could use smRows[i], mtRows[i], etc. here, since they're all the same
                      rbInvJoinObj_gl["new_inv_gl_stockGL"] =
                        glRows[i]["new_inv_gl_stock"];
                      rbInvJoinObj_gl["old_inv_gl_stockGL"] =
                        glRows[i]["old_inv_gl_stock"];
                    ...
                    searchResultsSplit =
                      searchResultsToString.split(splitRegex1);
                    console.log(
                      `searchResultsSplit.length==> ${searchResultsSplit.length}`
                    );
                    console.log(
                      `searchResultsSplit[0]==> ${searchResultsSplit[0]}`
                    );
                    console.log(
                    ...
                    for (let k = 0; k < searchResultsSplit.length; k++) {
                      let searchResultsSplitParsed = JSON.parse(
                        searchResultsSplit[k]
                      );
                      searchResultsSplitParsedArr.push(
                        searchResultsSplitParsed
                      );
                    }
                    console.log(
