...
export async function post(req, res, next) {
  ...
  SELECT * FROM rainbowcat WHERE shortName = '${vendorName}';
  `,
      (error, rows, fields) => {
        if (error) {
          ...
          console.log(
            `JSON.stringify(loadedTableRows)==> ${JSON.stringify(
              loadedTableRows
            )}`
          );
          console.log(
            `JSON.stringify(rainbowCatRows[0])==> ${JSON.stringify(
              rainbowCatRows[0]
            )}`
          );
          ...
          if (
            ...
            vndrWllnssMrgns !== "" &&
            vndrWllnssMrgns !== undefined
          ) {
            //don't run marginstager() if no
            ...
            console.log(
              `Object.keys(vndrWllnssMrgns_parsed)==> ${Object.keys(
                vndrWllnssMrgns_parsed
              )}`
            );
            console.log(
              `Object.values(vndrWllnssMrgns_parsed)==> ${Object.values(
                vndrWllnssMrgns_parsed
              )}`
            );
            ...
            for (let k = 0; k < stagedMargArr.length; k++) {
              console.log(
                `JSON.stringify(stagedMargArr[k])==> ${JSON.stringify(
                  stagedMargArr[k]
                )}`
              );
            }
