...
export async function post(req, res, next) {
  ...
  console.log(`JSON.stringify(req.files.popTableFile.name)==> ${JSON.stringify(req.files.popTableFile.name)}`)
  ...
  connection.query(popTableQuery, (error, response) => {
    ...
    res.json({
      "response from popTable": `~~~~~>> ${process.cwd()}/static/csv-to-insert/${fileName}.csv SAVED <<~~~~~`,
    })
  })
}