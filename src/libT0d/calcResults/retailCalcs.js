...
function retailCalcs(
  ...
  for (let i = 0; i < queryResArr.length; i++) {
      ...
      if (queryResArr[i]["dpt_number"] === stagedDptMargData[j]["dptNumb"]) {
        ...
        for (let k = 0; k < dptNameNumbMargMaster.length; k++) {
          ...
          if (reqdRtl < reqBody[`lowerCutoffRqdRtl${dptAbbr}`]) {
            //if req'd rtl is below lower cutoff
            //perform lower cutoff charm calcs
            if (reqdRtl % 1 > 0.8) {
              charm =
                reqdRtl -
                (reqdRtl % 1) +
                parseFloat(reqBody[`lowerCutoffCharm${dptAbbr}7`]);
            }
            if (reqdRtl % 1 < 0.8) {
              charm =
                reqdRtl -
                (reqdRtl % 1) +
                parseFloat(reqBody[`lowerCutoffCharm${dptAbbr}6`]);
            }
            if (reqdRtl % 1 < 0.6) {
              charm =
                reqdRtl -
                (reqdRtl % 1) +
                parseFloat(reqBody[`lowerCutoffCharm${dptAbbr}5`]);
            }
            if (reqdRtl % 1 < 0.5) {
              charm =
                reqdRtl -
                (reqdRtl % 1) +
                parseFloat(reqBody[`lowerCutoffCharm${dptAbbr}4`]);
            }
            if (reqdRtl % 1 < 0.4) {
              charm =
                reqdRtl -
                (reqdRtl % 1) +
                parseFloat(reqBody[`lowerCutoffCharm${dptAbbr}3`]);
            }
            if (reqdRtl % 1 < 0.3) {
              charm =
                reqdRtl -
                (reqdRtl % 1) +
                parseFloat(reqBody[`lowerCutoffCharm${dptAbbr}2`]);
            }
            if (reqdRtl % 1 < 0.2) {
              charm =
                reqdRtl -
                (reqdRtl % 1) +
                parseFloat(reqBody[`lowerCutoffCharm${dptAbbr}1`]);
            }
            if (reqdRtl % 1 < 0.1) {
              //change charm price to (#-1).99 if req'd rtl is #.00 -> #.10
              charm = reqdRtl - (reqdRtl % 1) - 0.01;
            }
            if (reqdRtl < 0.1) {
              //don't charm anything with req'd rtl less than 10 cents (doing this because of the veggie caps
              //Item ID = 686; alt ID = 089836060075)
              charm = reqdRtl;
            }
          } else {
            if (reqdRtl < reqBody[`upperCutoffRqdRtl${dptAbbr}`]) {
              //if req'd rtl is below upper charm cutoff
              //($12 for Grocery & $9999 for Wellness)
              if (reqdRtl % 1 > 0.856) {
                //bump anything from #.85+ and higher ==> #.99
                charm =
                  reqdRtl -
                  (reqdRtl % 1) +
                  parseFloat(reqBody[`defaultCharm${dptAbbr}4`]);
              }
              if (reqdRtl % 1 <= 0.855) {
                //bump anything from #.56 to #.85 ==> #.79 (Grocery); Wellness gets bumped
                //to #.99 for anything from #.56 to #.85 (because defaultCharm3 for Grocery is .79, but for Wellness it is .99)
                charm =
                  reqdRtl -
                  (reqdRtl % 1) +
                  parseFloat(reqBody[`defaultCharm${dptAbbr}3`]);
              }
              if (reqdRtl % 1 <= 0.55) {
                //bump anything from #.36 to #.55 ==> #.49
                charm =
                  reqdRtl -
                  (reqdRtl % 1) +
                  parseFloat(reqBody[`defaultCharm${dptAbbr}2`]);
              }
              if (reqdRtl % 1 <= 0.35) {
                //bump anything from #.10 to #.35 ==> #.29
                charm =
                  reqdRtl -
                  (reqdRtl % 1) +
                  parseFloat(reqBody[`defaultCharm${dptAbbr}1`]);
              }
              if (reqdRtl % 1 < 0.1) {
                //change charm price to (#-1).99 if req'd rtl is #.00 -> #.10
                charm = reqdRtl - (reqdRtl % 1) - 0.01;
              ...
              //if req'd rtl is above upper charm cutoff
              ...
              charm =
                reqdRtl -
                (reqdRtl % 1) +
                parseFloat(reqBody[`defaultCharm${dptAbbr}`]);
            }
          }
        }
      }
    ...
    if (parseFloat(catapultRtl) !== parseFloat(charm)) {
      //only include results in need of retail update
      let imwToPop = {};
      ...
      console.log(
        `JSON.stringify(reqBody) from retailCalcs==> ${JSON.stringify(reqBody)}`
      );
      ...
      } Rtl UPDT (pf5)`; //Power Field 5 - today's date
      ...
      if (
        reviewObj["cpltSKU"].toLowerCase() !== reviewObj["ediSKU"].toLowerCase()
      ) {
        //flag SKU mismatches, if they occur
      ...
      reviewObj["ediCost"] = `${
        queryResArr[i][`${reqBody.venCatPrefix}_cost`]
      }`;
      ...
      reviewObj["ediPrice"] = `${
        queryResArr[i][`${reqBody.venCatPrefix}_msrp`]
      }`;
