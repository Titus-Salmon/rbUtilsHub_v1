...
pow.POW_PK, pow.POW_ReceiveDate, pow.POW_InvoiceNumber,
...
pod.POD_PreDiscountTotalCommittedCost, pod.POD_ApplyDiscount, pod.POD_TotalCost, pod.POD_CostUpdateSource, pod.POD_ShipQuantity, pod.POD_Verified,
pod.POD_PkgCharges, pod.POD_UnitCharges, pod.POD_TotalCharges, pod.POD_PkgOrderCost, pod.POD_UnitOrderCost, pod.POD_TotalOrderCost,
pod.POD_CommitOnHand, pod.POD_CommitDivider,
...
where trim(pod.POD_TimeStamp) > '2021-06-20'
and trim(pow.POW_ReceiveDate) > '2021-06-20'
and trim(vim.ven_companyname) = 'EDI-KEHE'
and trim(pod.pod_received) = '1'
order by pow.POW_ReceiveDate, pow.POW_InvoiceNumber, vim.ven_companyname, vim.inv_scancode, vim.pi1_description, vim.pi2_description`;
</script>
